...
type
  TFrmATTAdjustTeam = class(TZSDForm)
    DBGrid1: TZSDDBGrid;
    dsView: TDataSource;
    cdsView: TZSDDataSet;
    Splitter1: TSplitter;
    ...
    cdsViewbNewTeam_: TStringField;
    cdsViewbStartDate_: TDateField;
    cdsViewbEndDate_: TDateField;
...
var
  FrmATTAdjustTeam: TFrmATTAdjustTeam;
...
procedure TFrmATTAdjustTeam.cdsViewAfterEdit(DataSet: TDataSet);
begin
  inherited;
  with cdsView do
  begin
...
procedure TFrmATTAdjustTeam.cdsViewAfterScroll(DataSet: TDataSet);
begin
  with cdsViewb do
  begin
    Close;
    CommandText := Format('Select * From ATT_ADJUSTTEAMB Where PID_=''%s''',
      [cdsView.FieldByName('ID_').AsString]);
    Open;
  end;
end;

procedure TFrmATTAdjustTeam.cdsViewbCalcFields(DataSet: TDataSet);
var
  cdsTemp: TZSDDataSet;
begin
  with DataSet do
...
procedure TFrmATTAdjustTeam.cdsViewBeforeDelete(DataSet: TDataSet);
begin
  with cdsViewb do
  begin
    First;
...
procedure TFrmATTAdjustTeam.cdsViewbNewRecord(DataSet: TDataSet);
begin
...
procedure TFrmATTAdjustTeam.cdsViewNewRecord(DataSet: TDataSet);
begin
  with DataSet do
  begin
...
procedure TFrmATTAdjustTeam.DBGrid2EditButtonClick(Sender: TObject);
var
  sField: String;
begin
...
procedure TFrmATTAdjustTeam.FormCreate(Sender: TObject);
begin
  inherited;
  with cdsView do
  begin
    Close;
    CommandText := 'Select First 10 * From ATT_ADJUSTTEAMA ';
    Open;
  end;
end;

procedure TFrmATTAdjustTeam.ZSDPostMsg(var AMsg: TWmCopyData);
begin
  inherited;
  case DM.ZSDMsgType of
    COST_ZSD_OPENID:
      begin
        ...
        begin
          Close;
          CommandText := Format('Select * From ATT_ADJUSTTEAMB Where Code_=''%s'' ',
            [DM.ZSDargs[0]]);
          Open;
        end;
      end;
...
procedure TFrmATTAdjustTeam.ZSDTool1Confirm(Sender: TObject);
begin
  with cdsView do
  begin
    Edit;
...
procedure TFrmATTAdjustTeam.ZSDTool1Revocation(Sender: TObject);
begin
  with cdsView do
  begin
    Edit;
...
procedure TFrmATTAdjustTeam.ZSDTool1Save(Sender: TObject);
begin
...
RegisterClass(TFrmATTAdjustTeam);

finalization

UnRegisterClass(TFrmATTAdjustTeam);
