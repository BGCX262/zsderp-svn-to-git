...
procedure TFrmQuoteB.cdsViewCalcFields(DataSet: TDataSet);
var
  cdsTemp: TZSDDataSet;
begin
  with DataSet do
...
procedure TFrmQuoteB.cdsViewNewRecord(DataSet: TDataSet);
begin
  if sPID = '' then
  begin
    DataSet.Cancel;
...
procedure TFrmQuoteB.DBGrid1EditButtonClick(Sender: TObject);
var
  sField: String;
begin
...
procedure TFrmQuoteB.FormCreate(Sender: TObject);
begin
  inherited;
  with cdsView do
  begin
    Close;
    CommandText := 'Select first 50* From QuoteB ';
    Open;
    CommandText := 'Select first 50* From QuoteB ';
  end;
...
procedure TFrmQuoteB.ZSDPostMsg(var AMsg: TWmCopyData);
begin
    ...
    COST_ZSD_OPENID:
      begin
        ...
        begin
          Close;
          CommandText :=
            Format('Select * From QuoteB Where PID_=''%s''', [sPID]);
          Open;
        end;
      end;
    COST_ZSD_SAVCODE:
...
RegisterClass(TFrmQuoteB);

finalization

UnRegisterClass(TFrmQuoteB);
